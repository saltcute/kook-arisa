    ...
    func: CommandFunction<BaseSession, any> = async (session: {
        ...
        for (const streamer of controller.activeStreamersArray) {
            ...
            card.addTextWithButton(
                `**${streamer.kasumi.me.username}#${streamer.kasumi.me.identifyNum}** By: (met)${streamer.INVITATION_AUTHOR_ID}(met)
...
message.on("ready", () => {
    message.client.events.button.registerActivator(
        "/admin/control/messageSession",
        async (
            event: {
                author: any;
            ...
            const user =
                await message.client.middlewares.AccessControl.global.group.getUser(
                    event.author
                );
            if (user.level > 2333) {
                ...
                if (!streamer) {
                    await message.client.API.message.create(
                        ...
                        event.channelId,
                        "Cannot find a streamer"
                    );
                    await message.client.API.message.update(
                        event.targetMsgId,
                        new Card().addText("Operation failed.")
                    );
                    return;
                }
                await message.client.API.message.create(
                ...
                const msg = await message.client.events.callback
                    .createAsyncCallback(
                        "message.text",
                        (e: { authorId: any }) => e.authorId == event.authorId,
                        (e: { content: any }) => e.content,
                        15 * 1000
                    )
                ...
                if (streamer) {
                    if (streamer.panel) {
                        await Promise.all(
                            streamer.panel?.panelChannelArray.map((v) =>
                                message.client.API.message.create(
                                    MessageType.MarkdownMessage,
                                    v,
                                    `管理员向此发送了一条消息：${msg}`
                                )
                ...
                await message.client.API.message.update(
                    event.targetMsgId,
                    new Card().addText("Operation completed.")
                );
            } else {
                await message.client.API.message.create(
                    MessageType.MarkdownMessage,
                    event.channelId,
                    `Your user group (${user.name}) is not permitted to do this!`,
                    undefined,
                    event.authorId
                );
                await message.client.API.message.update(
                    event.targetMsgId,
                    new Card().addText("Operation cancelled.")
                );
            }
        }
    );
