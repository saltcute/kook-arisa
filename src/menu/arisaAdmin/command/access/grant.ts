...
class GrantCommand extends BaseCommand {
    ...
    func: CommandFunction<BaseSession, any> = async (session) => {
        ...
        if (match && match[1]) {
            ...
            await session.send(
                `Confirm granting superuser rights to (met)${targetId}(met) (${user.username}#${user.identify_num})? (y/n)`
            );
            ...
            if (res == "timeout") {
                ...
                await session.send(
                    `Sucessfully granted superuser rights to (met)${targetId}(met) (${user.username}#${user.identify_num}). They are now in ${targetGroup.name} group now.`
                );
            } else {
