...
kill.on("ready", () => {
    kill.client.events.button.registerActivator(
        "/admin/control/killSession",
        async (event, data: KillDetail) => {
            const user =
                await kill.client.middlewares.AccessControl.global.group.getUser(
                    event.author
                );
            if (user.level > 2333) {
                ...
                if (!streamer) {
                    ...
                    await kill.client.API.message.update(
                        event.targetMsgId,
                ...
                const message = await kill.client.events.callback
                    .createAsyncCallback(
                        "message.text",
                        (e) => e.authorId == event.authorId,
                        (e) => e.content,
                        15 * 1000
                    )
                    .catch((_) => undefined);
                await streamer.disconnect(
                    `被管理员手动停止。${message ? `管理员附言：${message}` : ""}`
                );
                await kill.client.API.message.update(
                    event.targetMsgId,
