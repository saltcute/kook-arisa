        ...
        try {
            ...
            let tempRoleId;
            {
                await axios
                    ...
                    .catch((e) => {
                        throw new JoinChannelError(
                            2,
                            "Middleman is not able to join the server.",
                            e
                        );
                    });
            }
                ...
                streamerClientId = data.client_id;
            {
                ...
                if (err)
                    throw new JoinChannelError(
                        3,
                        "Cannot create temp role for middleman.",
                        err
                    );
            ...
            }
            {
                ...
                if (err)
                    throw new JoinChannelError(
                        4,
                        "Cannot assing permission to temp role for middleman.",
                        err
                    );
            }
            {
                ...
                if (err)
                    throw new JoinChannelError(
                        5,
                        "Cannot grant temp role to middleman.",
                        err
                    );
            }
            {
                ...
                if (err) {
                    if (textChannelId) {
                        const e = new JoinChannelError(
                            "6-1",
                            "Cannot change nickname for streamer.",
                            err
                        );
                        const card = new Card()
                            .addText(`错误信息：${e} `)
            ...
            }
            {
                ...
                if (err)
                    throw new JoinChannelError(
                        7,
                        "Cannot delete temp role for middleman.",
                        err
                    );
            }
            {
                ...
                if (err)
                    throw new JoinChannelError(
                        8,
                        "Cannot assign channel permission to streamer.",
                        err
                    );
            }
            {
                ...
                if (err)
                    throw new JoinChannelError(
                        9,
                        "Cannot update permission for streamer.",
                        err
                    );
            }
            {
                ...
                if (err)
                    throw new JoinChannelError(
                        10,
                        "Cannot assign channel permission to bot.",
                        err
                    );
            }
            {
                ...
                if (err)
                    throw new JoinChannelError(
                        11,
                        "Cannot update permission for bot.",
                        err
                    );
            }
            ...
            if (textChannelId) {
                const card = new Card()
                    .addTitle("无法加入语音频道")
                    .addText(
                        "可能原因：由于近期 KOOK 的 API 变化，机器人需要拥有「管理员」权限才能正常运行。"
                    );
                if (err instanceof JoinChannelError) {
                    card.addText(`错误信息：${err}`).addText(
                        `\`\`\`plain\n${err.err}\n\`\`\``
                    );
                }
                await client.API.message.create(
