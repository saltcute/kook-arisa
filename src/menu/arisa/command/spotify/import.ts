...
class ImportCommand extends BaseCommand {
    ...
    async replaceQueue(streamer: LocalStreamer, playlist: ISpotifyAlbumContent | ISpotifyPlaylistContent) {
        streamer.clearQueue();
    ...
    private tutorialCard(hasInput: boolean) {
        return new Card()
            .addTitle(hasInput ? "Spotify 专辑或歌单链接错误" : "请输入 Spotify 专辑或歌单链接")
            .addDivider()
            .addText(`请输入 Spotify 专辑或歌单链接，例如：
歌单：
\`\`\`plain
...
\`\`\``)
    ...
    func: CommandFunction<BaseSession, any> = async (session) => {
        ...
        let input = session.args[0], uri: string | undefined, list: ISpotifyAlbumContent | ISpotifyPlaylistContent | undefined;
