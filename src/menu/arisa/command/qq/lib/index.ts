...
export class QQMusic {
    ...
    updateCookie(cookie: string) {
        const obj: Object = Object.fromEntries(
            cookie
                .split(";")
                .map((v) => v.trim())
                .map((v) => {
        ...
        if (
            obj.hasOwnProperty("qqmusic_key") ||
            obj.hasOwnProperty("qm_keyst")
        ) {
    ...
    async search(
        ...
        page: number = 1,
        pageSize: number = 5
    ): Promise<QQMusic.API.Search> {
        const res = await qqmusic.api("search", {
            key: keywords,
            pageNo: page,
            pageSize,
        });
    ...
    async getAlbum(mid: string) {
        ...
        const songs: QQMusic.API.AlbumSongs = await qqmusic.api("album/songs", {
            mid,
        });
        return {
            songs,
