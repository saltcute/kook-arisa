...
class AppCommand extends BaseCommand {
    ...
    func: CommandFunction<BaseSession, any> = async (session) => {
        ...
        const { err, data } = await session.reply(new Card().addTitle("正在加载下一首歌..."));
        if (err) throw err;
        ...
        getChannelStreamer(session.guildId, session.authorId).then(async (streamer) => {
            ...
            if (upnext) {
                session.update(messageId, new Card()
                    .addTitle('Up Next')
                    .addText(`${upnext.meta.title} (font)${Time.timeToString(upnext.meta.duration / 1000)}(font)[secondary]`)
                    .addContext(upnext.meta.artists)
                )
            } else {
                session.update(messageId, new Card().addText("播放列表为空！"));
            }
        }).catch((e) => {
            switch (e.err) {
