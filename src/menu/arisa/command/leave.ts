...
class AppCommand extends BaseCommand {
    ...
    func: CommandFunction<BaseSession, any> = async (session) => {
        ...
        getChannelStreamer(session.guildId, session.authorId)
            .then(async (streamer) => {
                const { err, data: channelUserList } =
                    await client.API.channel.voiceChannelUserList(
                        session.channelId
                    );
                if (err) {
                    this.logger.error(err);
                    return session.reply(
                        new Card().addText("获取频道用户列表失败")
                    );
                }
                const userIDList = channelUserList.map((v) => v.id);
                if (
                    streamer.INVITATION_AUTHOR_ID == session.authorId ||
                    !userIDList.includes(streamer.INVITATION_AUTHOR_ID)
                ) {
