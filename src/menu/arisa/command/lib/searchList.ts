...
function cropCJKLatinString(str: string, maxLength: number = 30) {
    return getCJKLatinLength(str) > maxLength ? str.slice(0, maxLength - getCJKLength(str)) + "…" : str;
}

async function processNetease(song: Netease.songDetail, order: number): Promise<ProcessedNeteaseTrack> {
    let url;
    try {
        const originUrl = song.al.picUrl || (typeof song.al.pic_str == 'string' ? getResourceURL(song.al.pic_str) : getResourceURL(song.al.pic.toString()));
        if (originUrl) {
            try {
...
async function processQQ(song: QQMusic.Pattern.Song, order: number): Promise<ProcessedQQTrack> {
    let url;
    try {
...
function isNeteaseSong(song: Netease.songDetail | QQMusic.Pattern.Song): song is Netease.songDetail {
    return (song as Netease.songDetail).id !== undefined;
}

async function searchList(keyword: string, provider: string, songs: (Netease.songDetail | QQMusic.Pattern.Song)[], page: number = 1) {
    const card = new Card().setTheme(Card.Theme.INFO).addContext("部分 VIP 歌曲可能只能预览前 30s");
    const promises: Promise<(ProcessedNeteaseTrack | ProcessedQQTrack)>[] = [];
    let counter = 0;
    if (songs) {
        for (const song of songs) {
            counter++;
            if (isNeteaseSong(song)) promises.push(processNetease(song, counter));
        ...
        for (const processed of awaiteds) {
            ...
            card.addTextWithImage(`(font)${song.name}(font)[body]\n(font)${artists.join(", ")}(font)[secondary]`, {
                position: Card.Modules.AccessoryModes.LEFT, url: coverUrl
            });
            let sessionId;
            if (processed instanceof ProcessedNeteaseTrack) {
                sessionId = client.events.button.createSession('netease.queue.add', {
                    songId,
                    meta: {
                        title: song.name,
            ...
            card.addTextWithButton(`(font)添加「${cropCJKLatinString(song.name)}」到播放列表(font)[primary]`, {
                buttonContent: '添加',
...
client.events.button.registerActivator('search.nextPage', async (event: ButtonClickedEvent, data: {
    currentPage: number,
    keyword: string,
    provider: string
}) => {
    ...
    else if (data.provider == "qq") await client.API.message.update(event.targetMsgId, await searchQQ(data.keyword, data.currentPage));
});