    ...
    if (event.rawEvent.extra.type == "exited_channel") {
        ...
        if (streamer) {
            ...
            if (extra.body.user_id == streamer.kasumi.me.userId) {
                        ...
                        (id) =>
                            client.API.message.create(
                                ...
                                new Card()
                                    .addTitle("Notice | 注意")
                                    .addText(
                                        "机器人被动断开连接，机器人可能被踢出语音频道或经历了网络波动。如需结束推流，请使用指令 `/arisa leave` 停止点歌，不要将机器人直接踢出语音频道。"
                                    )
                            )
                    );
                ...
                const newStreamer = await controller.joinChannel(
                    streamer.TARGET_GUILD_ID,
                    streamer.TARGET_CHANNEL_ID,
                    streamer.INVITATION_AUTHOR_ID,
                    textChannelId
                );
                if (newStreamer) {
                    await playlist.user
                        .restore(newStreamer, newStreamer.INVITATION_AUTHOR_ID)
                        .catch((e) => {
