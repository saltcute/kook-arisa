...
import {
    ...
    ServerEvents,
    streamerDetail,
} from "webapp/src/components/cards/types";
import api from "./api";
...
app.ws("/", (ws: WebSocket) => {
        ...
        try {
            ...
            switch (payload.t) {
                case ClientEvents.GET_USER_ID: // Get user ID
                    ...
                    userId = user.data.id;
                    break;
                case ClientEvents.PLAYBACK_PAUSE_RESUME: {
                    // Pause and resume
                ...
                case ClientEvents.PLAYBACK_NEXT_PREVIOUS: {
                ...
                case ClientEvents.PLAYBACK_MOVE_QUEUE: {
                    // Move queue items
                    const streamer = getAllStreamers()[payload.d.streamerIndex];
                    if (streamer) {
                ...
                case ClientEvents.PLAYBACK_SHUFFLE_QUEUE: {
                    // Shuffle queue
                    const streamer = getAllStreamers()[payload.d.streamerIndex];
                    if (streamer) {
                        streamer.shuffle();
                ...
                case ClientEvents.PLAYBACK_CYCLE_MODE: {
                    // Change cycle mode
                    const streamer = getAllStreamers()[payload.d.streamerIndex];
                    if (streamer) {
                ...
                case ClientEvents.PLAYBACK_PLAY_SONG: {
                    // Play song
                    const streamer = getAllStreamers()[payload.d.streamerIndex];
                    if (streamer) {
                        ...
                        switch (data.type) {
                            ...
                            case "bilibili": {
                                if (Date.now() - lastBilibiliRequest < 1 * 1000)
                                    break;
                                if (streamer instanceof LocalStreamer) {
                                    ...
                                    >;
                                    if (bvid)
                                        video = await getVideoDetail(bvid);
                                    if (bvid && video) {
                ...
                case ClientEvents.PLAYBACK_JUMP_TO_PERCENT: {
                    // Jump to percentage
                    const streamer = getAllStreamers()[payload.d.streamerIndex];
                    if (streamer) {
                ...
                case ClientEvents.SELECT_GUILD: {
                    // Select guild
                    // guildId = payload.d.guildId;
                    // getChannelStreamer()
                    break;
                }
                case ClientEvents.CLIENT_PING: {
                    // Ping
                    ws.send(
                        JSON.stringify({
                ...
                case ClientEvents.PLAYBACK_VOLUME: {
                    // Change volume
                    const streamer = getAllStreamers()[payload.d.streamerIndex];
                    if (streamer) {
        ...
        if (userId) {
            ...
            if (streamers) {
                ...
                for (const streamer of streamers) {
                    ...
                    array = queue
                        .filter((v) => v.extra)
                        .map((v) => v.extra) as playback.extra[];
    ...
    function getChannelStreamer() {
        return controller.activeStreamersArray.filter((v) =>
            v.audienceIds.has(userId)
        );
    }
...
app.get("/login", async (req, res) => {
    res.redirect(
        `https://www.kookapp.cn/app/oauth2/authorize?id=12273&client_id=${await client.config.getOne("kookClientID")}&redirect_uri=${encodeURIComponent((await client.config.getOne("webuiUrl")).toString())}&response_type=code&scope=get_user_info%20get_user_guilds`
    );
});
...
client.config.get("internalWebuiPort").then(({ internalWebuiPort }) => {
    app.listen(internalWebuiPort, async () => {
        client.logger.info(
            `Webui start listening on port ${await client.config.getOne("internalWebuiPort")}`
        );
        client.logger.info(
            `Access webui at ${await client.config.getOne("webuiUrl")} or http://localhost:${await client.config.getOne("internalWebuiPort")}`
        );
    });
});
