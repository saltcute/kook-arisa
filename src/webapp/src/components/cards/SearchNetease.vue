...
const akarin =
    "https://img.kookapp.lolicon.ac.cn/assets/2022-07/vlOSxPNReJ0dw0dw.jpg";
...
async function getNeteaseSearch(
    keyword: string
...
function search() {
    ...
    if (searchResultElement?.getAttribute("aria-busy") != "true") {
        ...
        switch (activeTab.value) {
            case "netease":
                getNeteaseSearch(searchInput.value)
                    .then(async (res) => {
                        if (res) {
                            searchResult = res;
                ...
                break;
            case "qqmusic":
                getQQMusicSearch(searchInput.value)
                    .then(async (res) => {
                        if (res) {
            ...
            case "bilibili":
                ...
                const pattern =
                    /(?:https?:\/\/(?:www|m).bilibili.com\/video\/)?(BV[0-9A-Za-z]{10})/gm;
                ...
                if (bvid) {
                    ...
                    notify({
                        ...
                        text: t(
                            "notification.search.error.bilibiliWrongLinkPattern"
                        ),
                    });
                }
        }
...
function addNeteaseTrack(song: Netease.songDetail) {
    ...
    if (backend.currentStreamer) {
        notify({
            group: "search",
            title: t("notification.search.success.title"),
            text: t("notification.search.success.addedToPlaylist", {
                name: song.name,
            }),
        });
    } else {
        notify({
            ...
            text: t("notification.search.warning.noStreamers", {
                name: song.name,
            }),
        });
    }
        ...
        <article>
            <header>
                ...
                <label for="track-name">
                    <span
                        v-if="activeTab == 'netease' || activeTab == 'qqmusic'"
                    >
                        {{ t("desc.search.title.searchForTrack") }}
                    </span>
                    <span v-else-if="activeTab == 'bilibili'">{{
                        t("desc.search.title.playBilibiliVideo")
                    }}</span>
                    <span v-else>{{ t("desc.search.title.unknown") }}</span>
                    <i
                        class="iconfont icon-arisa-wangyiyun"
                        :class="{ active: activeTab == 'netease' }"
                        @click="switchTabTo('netease')"
                    ></i>
                    <i
                        class="iconfont icon-arisa-QQyinleshiliangtubiao"
                        :class="{ active: activeTab == 'qqmusic' }"
                        @click="switchTabTo('qqmusic')"
                    ></i>
                    <i
                        class="iconfont icon-arisa-bilibili"
                        :class="{ active: activeTab == 'bilibili' }"
                        @click="switchTabTo('bilibili')"
                    ></i>
                    <input
                        v-model="searchInput"
                        type="search"
                        ...
                        :placeholder="
                            activeTab == 'bilibili'
                                ? t('desc.search.boxDesc.bilibiliVideo')
                                : t('desc.search.boxDesc.trackName')
                        "
                        @keyup.enter="search()"
                    />
                </label>
            ...
            <div class="search-result">
                <article
                    ...
                    v-for="song of searchResult"
                >
                    <span v-if="isNeteaseSong(song)">
                        <img
                            class="cover"
                            :src="(song as Netease.songDetail).al.picUrl"
                        />
                        <span class="track-meta">
                            <p class="title">
                                {{ (song as Netease.songDetail).name }}
                            </p>
                            <p class="artists">
                                {{
                                    (song as Netease.songDetail).ar
                                        .map((v) => v.name)
                                        .join(", ")
                                }}
                            </p>
