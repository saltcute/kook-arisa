...
<template>
    <header>
        <nav>
            <ul>
                <li>
                    <strong class="title">{{ t("desc.header.title") }}</strong>
                </li>
            </ul>
            <ul>
                <li>
                    <details role="list" class="language-selector">
                        <summary aria-haspopup="listbox">
                            <i
                                class="language-name flag"
                                v-emoji="
                                    getFlagEmoji($i18n.locale.split('-')[1])
                                "
                            ></i>
                            <span class="language-current">{{
                                t(`desc.language.name.self`)
                            }}</span>
                        </summary>
                        <ul role="listbox" class="dropdown">
                            ...
                            >
                                <div class="language-name original">
                                    <span>{{
                                        t(
                                            `desc.language.name.original.${locale}`
                                        )
                                    }}</span>
                                </div>
                                <div class="language-name translated">
                                    {{
                                        t(
                                            `desc.language.name.translated.${locale}`
                                        )
                                    }}
                                </div>
                            </li>
                ...
                <li>
                    ...
                    <a v-else="callbackUrl" :href="callbackUrl" role="button">{{
                        $t("action.login")
                    }}</a>
                </li>
                <li v-if="user">
                    <a @click="logout" href="#" role="button">{{
                        $t("action.logout")
                    }}</a>
                </li>
            </ul>
